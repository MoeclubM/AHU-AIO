...
class SynjonesClient {
  ...
  SynjonesClient._internal() {
    _casDio = Dio(BaseOptions(
      connectTimeout: const Duration(seconds: 15),
      receiveTimeout: const Duration(seconds: 15),
      headers: {'User-Agent': _ua},
    ));
    _ycardDio = Dio(BaseOptions(
      baseUrl: _ycardBase,
      connectTimeout: const Duration(seconds: 15),
      receiveTimeout: const Duration(seconds: 15),
      headers: {
  ...
  Future<LoginResult> casLogin({
    ...
    try {
      ...
      final ltMatch = RegExp(r'name="lt"\s+value="([^"]+)"')
      ...
      final devResp = await _casDio.post(
        '$_casBase/cas/device',
        data: 'ul=${username.length}&pl=${password.length}'
      ...
      var devInfo = (devResp.data is Map
          ? devResp.data['info']
          : null)
          ?.toString();
      if (devInfo == 'unbind') {
      ...
      if (devInfo != 'ok') {
        return LoginResult(
          success: false,
          message: '设备校验失败: $devInfo',
        );
      }

      // 4) POST login form
      final formBody = 'rsa=${Uri.encodeComponent(rsa)}'
  ...
  Future<LoginResult> _exchangeToken(String ticket) async {
    try {
      final resp = await _ycardDio.post(
        ...
        data: 'username=${Uri.encodeComponent(ticket)}'
  ...
  static List<int> _desProcessBlock(
    Uint8List block,
    List<Uint8List> subKeys,
  ) {
    // Initial Permutation
  ...
  static List<int> _feistel(List<int> r, Uint8List subKey) {
    ...
    for (var i = 0; i < 8; i++) {
      ...
      final col = (group[1] << 3) | (group[2] << 2) | (group[3] << 1) | group[4];
  ...
  static const _sBoxes = [
    ...
    // S5
    [2,12,4,1,7,10,11,6,8,5,3,15,13,0,14,9,14,11,2,12,4,7,13,1,5,0,15,10,3,9,8,6,4,2,1,11,10,13,7,8,15,9,12,5,6,3,0,14,9,14,15,5,2,8,12,3,7,0,4,10,1,13,11,6],
    // S6
    [4,2,1,11,10,13,7,8,15,9,12,5,6,3,0,14,0,12,7,11,10,1,13,14,5,8,15,6,2,3,9,4,1,14,4,11,8,12,6,2,15,9,7,3,10,5,0,13,6,1,13,8,11,4,2,7,15,10,9,5,3,14,12,0],
    // S7
    [13,2,8,4,6,15,11,1,10,9,3,14,5,0,12,7,1,15,13,8,10,3,7,4,12,5,6,11,0,14,9,2,7,11,4,1,9,12,14,2,0,6,10,13,15,3,5,8,2,1,14,7,4,10,8,13,15,12,9,0,3,5,6,11],
    // S8
    [1,15,13,8,10,3,7,4,12,5,6,11,0,14,9,2,7,11,4,1,9,12,14,2,0,6,10,13,15,3,5,8,4,1,14,8,13,6,2,11,15,12,9,7,3,10,5,0,15,12,8,2,4,9,1,7,5,11,3,14,10,0,6,13],
  ];
}
