...
void main() {
  group('CasNativeClient login behavior', () {
    test('login clears existing CAS session cookie before authentication', () async {
    ...
    test('login throws error when password is wrong, never falsely succeeds', () async {
      final dio = Dio();
      final cookieJar = CookieJar();
      ...
      expect(
        () async => await client.login(
          loginUri: Uri.parse('https://one.ahu.edu.cn/cas/login?service=https%3A%2F%2Fjw.ahu.edu.cn%2Fstudent%2Fsso%2Flogin'),
          username: '20230001',
    ...
    test('login succeeds when password is correct and follows redirects', () async {
      ...
      dio.httpClientAdapter = MockHttpClientAdapter((options) async {
        ...
        if (options.method == 'GET' && uri.path == '/cas/login') {
          ...
          return ResponseBody.fromString(
            ...
            302,
            headers: {
              'location': ['https://jw.ahu.edu.cn/student/sso/login?ticket=ST-mock-ticket-123'],
            },
          );
      ...
      expect(result.observedUris.any((u) => u.queryParameters['ticket'] == 'ST-mock-ticket-123'), isTrue);
    });
  });
}
