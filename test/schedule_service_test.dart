...
void main() {
  group('ScheduleService.buildWeekSchedule', () {
    ...
    test(
      'filters entries by selected week and marks current and honor flags',
      () {
        service.replaceClassesForTest([
          {
            'course': {'nameZh': '数学分析', 'courseType': '荣誉课程'},
    ...
    test('derives week number from course date when indices are missing', () {
      ...
      expect(entry.isCurrentWeek, isFalse, reason: '当前周不同于课程所在周，因而不应高亮');
    ...
    test('parses textual week indices including odd/even hints', () {
      ...
      expect(
        evenWeekSchedule[DateTime.thursday],
        isEmpty,
        reason: '单周课程在双周不应展示',
      );
    });
