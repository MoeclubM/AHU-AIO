...
void main() {
  group('MainLayoutScreen calculateSubTabVisibility', () {
    test('sub-tab bar stays at 1.0 (continuous display, never hides) for pages 0 to 2', () {
      // 0: 微教务
      expect(MainLayoutScreen.calculateSubTabVisibility(0.0), 1.0);
      expect(MainLayoutScreen.calculateSubTabVisibility(0.25), 1.0);
      expect(MainLayoutScreen.calculateSubTabVisibility(0.5), 1.0,
      ...
      expect(MainLayoutScreen.calculateSubTabVisibility(1.5), 1.0,
    ...
    test('sub-tab bar smoothly retracts between page 2 (一卡通) and page 3 (设置)', () {
      // 从一卡通(2.0)滑动向设置(3.0)
      // t = 0.25 (page = 2.25) -> 1.0 - 2.0 * 0.25 = 0.5
      expect(MainLayoutScreen.calculateSubTabVisibility(2.25), closeTo(0.5, 0.001));
    ...
    test('sub-tab bar stays hidden (0.0) on page 3 (设置) and beyond', () {
      ...
      expect(MainLayoutScreen.calculateSubTabVisibility(3.5), 0.0,
          reason: '过度滑动设置页时保持隐藏');
    });

    test('sub-tab bar stays at 1.0 for negative overshoot at page 0', () {
      expect(MainLayoutScreen.calculateSubTabVisibility(-0.2), 1.0,
